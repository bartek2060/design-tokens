import { DesignToken, FileBuild, TokenSettings, UtilitySettings } from "../types";
import { SCSS_UTILITY_BREAKPOINT_SEPARATOR, SETTINGS_KEY } from "../constants";
import { cssKey, ensureArray } from "./generateStyles";

type ProcessedEntry = {
  key: string;
  nestedKey?: string;
  value: string | DesignToken;
  settings?: TokenSettings;
};

const generatePreview = (value: string | DesignToken) => {
...
const generateUtilityDocs = (
  utility: UtilitySettings | UtilitySettings[] | undefined,
  tokenPath: string[],
  settings: TokenSettings
) => {
  if (!utility) return "";

  const utilities = ensureArray(utility);
  const tokenValueName = tokenPath[tokenPath.length - 1];

  const utilityClasses = utilities.map((util: UtilitySettings) => {
    const classes = [`${cssKey(".", [util.className, tokenValueName])}`];
    ...
    if (settings.utilityBreakpoints && typeof settings.utilityBreakpoints !== "string") {
      ...
      Object.keys(breakpoints).forEach((breakpoint: string) => {
        classes.push(cssKey(".", [breakpoint + SCSS_UTILITY_BREAKPOINT_SEPARATOR + util.className, tokenValueName]));
      });
    }

    return {
      classes,
      properties: ensureArray(util.property),
    };
  });
  ...
  if (tokenPath.length === 1) {
    return `
      <div class="utility-docs">
        ...
        ${utilities
          .map(
            (util: UtilitySettings, index: number) => `
        ...
        ${utilityClasses
          .map((util) =>
            util.classes
              .map(
                (className) =>
                  `<code class="utility-class-compact" title="Sets: ${util.properties.join(", ")}">${className}</code>`
              )
              .join("")
          )
          .join("")}
...
const shouldShowSection = (name: string, tokens: Record<string, any>): boolean => {
  if (name === SETTINGS_KEY) return false;
  return Object.entries(tokens).some(([key]) => key !== SETTINGS_KEY);
};

const generateTokenSection = (name: string, tokens: Record<string, any>, settings: TokenSettings | undefined) => {
  if (!shouldShowSection(name, tokens)) return "";

  const tokenEntries = Object.entries(tokens).filter(([key, value]) => {
    return key !== SETTINGS_KEY && (typeof value === "string" || (typeof value === "object" && value !== null));
  });

  const processedEntries: ProcessedEntry[] = tokenEntries
    .map(([key, value]) => {
      if (typeof value === "object" && value !== null) {
        const nestedSettings = { ...settings, ...value[SETTINGS_KEY] } as TokenSettings | undefined;
        return Object.entries(value)
          .filter(([nestedKey]) => nestedKey !== SETTINGS_KEY)
          .map(([nestedKey, nestedValue]) => ({
            key,
            nestedKey,
            value: nestedValue as string | DesignToken,
      ...
      return [
        {
          key,
          value: value as string | DesignToken,
          settings,
        },
      ];
    })
      ...
      <div class="token-grid">
        ${processedEntries
          .map((entry) => {
            ...
            const preview = generatePreview(entry.value as string);
...
export const generateDocs = (tokens: DesignToken): FileBuild => {
  const html = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
      <div class="container">
        ...
        ${Object.entries(tokens)
          ...
          .map(([name, value]) =>
            generateTokenSection(name, value as Record<string, any>, (tokens[name] as any)?.__settings)
          )
          .join("")}
      </div>
    </body>
